import search.Travel;
import search.HashtagList;
import java.util.*;
import java.io.*;
...
public class Search extends HttpServlet {
	...
	try {
		...
		sql = "select * ,DATE_FORMAT(travel_dateStart,'%d  %b  %Y') as'dateStart', DATE_FORMAT(travel_dateEnd,'%d  %b  %Y') as 'dateEnd' from travel t,member m where t.member_id=m.member_id and t.travel_status='1' order by t.travel_dateAnnounce desc";
        ...
        while (rs.next()) {
			...
			searchlist.setStartDate(rs.getString("dateStart"));
			searchlist.setEndDate(rs.getString("dateEnd"));
