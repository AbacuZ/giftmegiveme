  ...
  private final DatabaseConnection openConnection = new DatabaseConnection();
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			statement = connect.prepareStatement("select *,DATE_FORMAT(travel_dateAnnounce,'%d/%l/%Y %H:%m:%s')as'dateAnnounce' from travel  where member_id=? order by travel_dateAnnounce desc");
			...
			while(rs.next()){
				...
				travel.setCurrentDate(rs.getString("dateAnnounce"));
				list.add(travel);
			}
